...
<script src="<?php $options->adminStaticUrl('js', 'admin.js?v=' . $suffixVersion);?>"></script>
<script>
(function() {
    $(document).ready(function() {
        ...
        if ($('.typecho-login').length == 0) {
            $('a').each(function() {
                ...
                if ((href && href[0] == '#') ||
                    /^<?php echo preg_quote($options->adminUrl, '/
                    '); ?>.*$/.exec(href) ||
                    /^<?php echo substr(preg_quote(Typecho_Common::url('s', $options->index), '/
                    '), 0, -1); ?>action\/[_a-zA-Z0-9\/]+.*$/.exec(href)) {
                    return;
                }
